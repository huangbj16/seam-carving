...
void MyMat::ShowX(){
	Mat M(ori_rows, 2*ori_cols-cols, CV_8UC3, Scalar::all(0));
	int ori_row, ori_col;
	for(int row=0; row<rows; ++row){
		for(int col = 0; col < cols; ++col){
			ori_row = array[col][row].y;
			ori_col = array[col][row].x;
			M.at<Vec3b>(ori_row, ori_col + mark[ori_col][ori_row]) = array[col][row].c;
		}
	}
	for(int i=0; i<ori_cols; ++i){
		for(int j=0; j<ori_rows; ++j){
			if(mark[i][j]){
				M.at<Vec3b>(j, i+mark[i][j]) = ori_mat.at<Vec3b>(j, i);
				M.at<Vec3b>(j, i+mark[i][j]-1) = ori_mat.at<Vec3b>(j, i);
			}
		}
	}
	imwrite("ori.png", M);
...
void MyMat::ShowY(){
	Mat M(2*ori_rows-rows, ori_cols, CV_8UC3, Scalar::all(0));
	int ori_row, ori_col;
	for(int row=0; row<rows; ++row){
		for(int col = 0; col < cols; ++col){
			ori_row = array[col][row].y;
			ori_col = array[col][row].x;
			M.at<Vec3b>(ori_row+mark[ori_col][ori_row], ori_col) = array[col][row].c;
		}
	}
	for(int i=0; i<ori_cols; ++i){
		for(int j=0; j<ori_rows; ++j){
			if(mark[i][j]){
				M.at<Vec3b>(j+mark[i][j], i) = ori_mat.at<Vec3b>(j, i);
				M.at<Vec3b>(j+mark[i][j]-1, i) = ori_mat.at<Vec3b>(j, i);
			}
		}
	}
	imwrite("ori.png", M);
...
void MyMat::CutX(){
	...
	for(int y=rows-1; y>=0; --y){
		mark[array[minx][y].x][array[minx][y].y] = 1;
...
void MyMat::CutY(){
	...
	for(int x=cols-1; x>=0; --x){
		mark[array[x][miny].x][array[x][miny].y] = 1;
...
void MyMat::ShowseamX(){
	Mat M(ori_rows, 2*ori_cols-cols, CV_8UC3, Scalar::all(255));
	int ori_row, ori_col;
	for(int row=0; row<rows; ++row){
		for(int col = 0; col < cols; ++col){
			ori_row = array[col][row].y;
...
void MyMat::ShowseamY(){
	Mat M(2*ori_rows-rows, ori_cols, CV_8UC3, Scalar::all(255));
	int ori_row, ori_col;
	for(int row=0; row<rows; ++row){
		for(int col = 0; col < cols; ++col){
			ori_row = array[col][row].y;
...
void MyMat::ScanY(){
	for(int i=1; i<ori_rows; ++i){
		for(int j=0; j<ori_cols; ++j){
			mark[j][i] += mark[j][i-1];
		}
	}
}
